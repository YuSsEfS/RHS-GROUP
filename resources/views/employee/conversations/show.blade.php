...
@section('title', 'Conversation interne')
@section('brand', 'RHS Employe')
@section('brand_sub', 'Messagerie')
@section('page_title', 'Conversation interne')
@section('page_copy', 'Consultez le fil complet, repondez rapidement et joignez vos documents depuis une interface type messagerie.')
...
@section('content')
        ...
        <aside class="rhs-chat-sidebar rhs-chat-card">
            ...
            <div class="rhs-chat-list" data-chat-list>
                @foreach($conversations as $listConversation)
                    ...
                    @endphp
                    <a
                        href="{{ route('employee.messages.show', $listConversation) }}"
        ...
        <section class="rhs-chat-main rhs-chat-card">
            <div class="rhs-chat-header">
                <div class="rhs-chat-header-main">
                    <span class="rhs-chat-avatar rhs-chat-avatar-lg">
                        @if(!$conversation->isGroup() && $otherParticipant?->profile_photo_url)
                            <img src="{{ $otherParticipant->profile_photo_url }}" alt="{{ $conversationName }}">
                        @else
                ...
                <div class="rhs-chat-header-actions">
                    <a href="{{ route('employee.messages.index', ['create' => 1]) }}" class="rhs-chat-action-link">Nouvelle</a>
                    <a href="{{ route('employee.messages.index', ['empty' => 1]) }}" class="rhs-chat-icon-btn" title="Fermer la conversation" aria-label="Fermer la conversation">x</a>
            ...
            <div
                ...
                data-chat-refresh-url="{{ route('employee.messages.items.index', $conversation) }}"
            ...
            <div class="rhs-chat-composer">
                <form method="POST" action="{{ route('employee.messages.store', $conversation) }}" class="rhs-chat-composer-form" enctype="multipart/form-data" data-rhs-chat-composer>
                    @csrf
                    <div class="rhs-chat-composer-box">
                        ...
                        <input id="employee-chat-attachment" name="attachments[]" type="file" multiple class="rhs-chat-attach-input" data-rhs-chat-file-input>
                        <label for="employee-chat-attachment" class="rhs-chat-tool-btn" title="Ajouter une piece jointe">
            ...
            <div class="rhs-chat-side-section">
                ...
                <div class="rhs-chat-side-attachment-data" hidden>
                    @foreach($attachments as $attachment)
                        @php($attachmentUrl = route('employee.messages.attachment', $attachment))
                        @php($previewUrl = ($attachment->isWordAttachment() || $attachment->isSpreadsheetAttachment()) ? route('employee.messages.preview', $attachment) : null)
                        <span
    ...
    @include('partials.rhs-chat-modals', [
        'bulkDownloadUrl' => route('employee.messages.attachments.download', $conversation),
    ])
@endsection
