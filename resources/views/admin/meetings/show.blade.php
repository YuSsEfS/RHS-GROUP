...
@section('page_subtitle','Detail de la reunion et participants notifies.')

@section('top_actions')
<a href="{{ route('admin.meetings.edit', $meeting) }}" class="btn btn-primary">Modifier</a>
<a href="{{ route('admin.meetings.index') }}" class="btn btn-ghost">Retour</a>
@endsection
      ...
      <div class="meeting-participant-list">
        @forelse($meeting->users as $participant)
          <div class="meeting-participant-card">
            ...
            <span class="meeting-status {{ $participant->pivot->notification_read_at ? 'is-completed' : 'is-cancelled' }}">
              {{ $participant->pivot->notification_read_at ? 'Vue' : 'Non lue' }}
            </span>
          </div>
        @empty
          <div class="meeting-empty">
            <strong>Aucun participant</strong>
            <span>Ajoutez des participants pour notifier l equipe.</span>
          </div>
        @endforelse
      </div>
    </div>
