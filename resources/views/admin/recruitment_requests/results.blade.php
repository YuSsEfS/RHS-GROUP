...
<div class="panel">
  <div class="panel-head">
    ...
    <div class="panel-tools">
      <form method="GET" class="table-controls" action="{{ route('admin.recruitment_requests.results', $recruitmentRequest) }}" autocomplete="off">
        <div class="table-filter">
          <label for="offer" class="sr-only">Offre</label>
          <select name="offer" id="offer" onchange="this.form.submit()">
            <option value="all" {{ request('offer', $recruitmentRequest->job_offer_id ?: 'all') === 'all' ? 'selected' : '' }}>
              Toutes les offres
            </option>
            @foreach(($offers ?? collect()) as $o)
              <option value="{{ $o->id }}" {{ (string) request('offer', $recruitmentRequest->job_offer_id ?: 'all') === (string) $o->id ? 'selected' : '' }}>
                {{ $o->title }}
              </option>
            @endforeach
          </select>
        ...
        <div class="table-ctrl-actions">
          ...
          <a class="btn btn-ghost btn-sm" href="{{ route('admin.recruitment_requests.results', $recruitmentRequest) }}">Réinitialiser</a>
        </div>
      </form>
    </div>
  </div>
                  ...
                  <div class="match-breakdown">
                    @foreach($fullBreakdown as $key => $value)
                      <span class="match-tag">
                        {{ $breakdownLabels[$key] ?? ucfirst(str_replace('_', ' ', $key)) }} : {{ rtrim(rtrim(number_format((float) $value, 2, '.', ''), '0'), '.') }}
                      </span>
                    @endforeach
                  </div>
                @endif
